...
for (file in list.files(path = getwd())) {
  if (endsWith(file, "tsv")) {
    ...
    df = df %>% group_by(Algorithm) %>% summarise(AUROC = max(AUROC)) %>% mutate(Combination = str_wrap(str_replace(file, ".tsv", ""), width= 2))
    list_of_dfs[[i]] <- df
    i <- i + 1
  }
}
...
df %>%
  ggplot(aes(x = Combination, y = AUROC, color = Algorithm, fill = Algorithm)) +
  theme_bw() +
  geom_point() +
...
ggsave("graphs/index_five.png", height = 7, width = 15)
